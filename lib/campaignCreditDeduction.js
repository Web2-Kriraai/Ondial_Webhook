...
async function resolveRatePerMinute(db, user, campaign, callLogDoc = null) {
    ...
    if (!billingOverrideApplied) {
        try {
            const destPhone = extractDestinationPhone(callLogDoc || {});
...
async function tryDeductCampaignCallCredits({
    ...
    const callLogFilter = { $or: [{ lead_id: callId }, { call_id: callId }] };
    let callLogDoc = await db.collection(CALLLOGS_COLLECTION).findOne(callLogFilter);
    if (!callLogDoc) {
    ...
    try {
        ...
        if ((user.credits || 0) < cost) {
            await db.collection(CALLLOGS_COLLECTION).updateOne(callLogFilter, {
                $set: {
        ...
        await db.collection("credittransactions").insertOne({
            ...
            amount: -cost,
            balanceAfter,
            description: `Call usage - ${dur}s (webhook)`,
            reference: {
                billingKey,
