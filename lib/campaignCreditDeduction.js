...
async function tryDeductCampaignCallCredits({
    ...
    try {
        ...
        // Tenant CRM webhooks (outbound billed + credits.usage). Inbound billed is handled in Ondial.
        if (billingKind === "campaign" && !inboundLog) {
            try {
                const { notifyTenantCallBilled } = require("./notifyTenantCallStatus");
                let ownerId = user._id;
                void notifyTenantCallBilled({
                    userId: ownerId,
                    contactId: contactId || null,
                ...
                logger.warn("[Credit] tenant billed notify failed", {
