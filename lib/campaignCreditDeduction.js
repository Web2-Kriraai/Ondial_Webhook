...
async function resolveRatePerMinute(db, user, campaign) {
...
async function tryDeductCampaignCallCredits({
    ...
    const callLogDoc = await db.collection(CALLLOGS_COLLECTION).findOne(callLogFilter);
    ...
    try {
        ...
        const rates = await resolveRatePerMinute(db, user, campaign);
