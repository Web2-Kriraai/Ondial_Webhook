...
// Country-based rates — enable after lib/countryCallingRates.js + lib/countryFromPhone.js are deployed
// const {
//     applyCountryRateToPackageRate,
//     loadCountryCallingRates,
// } = require("./countryCallingRates");
// const { countryIsoFromPhone, extractDestinationPhone } = require("./countryFromPhone");
...
/** Caller/callee phone from InboundConversation event history (for country rate when enabled). */
...
//         const to = d.to || d.To_number || d._raw?.call?.to;
//         if (from && String(from).trim()) return String(from).trim();
//         if (to && String(to).trim()) return String(to).trim();
//     }
//     return null;
// }
...
async function resolveRatePerMinute(db, user, campaign, _callLogDoc = null) {
    ...
    // Country-based rate multiplier — uncomment when countryCallingRates is deployed
    // if (!billingOverrideApplied) {
    //     try {
    //         const destPhone =
    //             extractDestinationPhone(_callLogDoc || {}) || phoneFromInboundEvents(_callLogDoc);
    //         const defaultCountry =
    //             campaign?.companyCountryIso ||
    //             campaign?.contactImportCountryIsoOverride ||
    //             "IN";
    //         destinationCountryIso = countryIsoFromPhone(destPhone, defaultCountry);
...
async function tryDeductCampaignCallCredits({
    ...
    const markCallLogCreditFields = (fields) =>
        db.collection(logCollection).updateOne(callLogFilter, { $set: fields });
    ...
    try {
        ...
        const rates = await resolveRatePerMinute(db, user, campaign, callLogDoc);
