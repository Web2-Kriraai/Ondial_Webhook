...
/** Caller/callee phone from InboundConversation event history for country rate. */
...
async function resolveRatePerMinute(db, user, campaign, callLogDoc = null) {
    ...
    if (!billingOverrideApplied) {
        try {
            const destPhone =
                extractDestinationPhone(callLogDoc || {}) || phoneFromInboundEvents(callLogDoc);
