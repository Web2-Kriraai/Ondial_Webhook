...
async function triggerCallAnalysis(callId, options = {}) {
    ...
    // Foreign outbound: Twilio + Telnyx. India/pool uses dialer UUID path separately when enabled.
    if (provider !== "twilio" && provider !== "telnyx") {
        logger.info("[Analysis] Skipped — foreign outbound only (twilio|telnyx)", {
            callId: id,
            callProvider: provider,
        });
        return { triggered: false, reason: "foreign_outbound_only" };
    }
