...
async function resolveAnalysisPayload(callId) {
    ...
    if (campaignId && /^[a-f0-9]{24}$/i.test(campaignId)) {
        ...
        if (campaignDoc && !campaignDoc.companyName && campaignDoc.companyId) {
            try {
                const company = await db.collection('companies').findOne({ _id: new ObjectId(campaignDoc.companyId) });
                if (company) {
                    campaignDoc.companyName = company.name;
                }
            } catch (cErr) {
                logger.warn("[Analysis] Failed to populate companyName for analysis payload", { error: cErr.message });
            }
        }
    }
