...
function buildCallLogLookupFilter(callId) {
    ...
    const clauses = [{ lead_id: id }, { call_id: id }, { call_unique_id: id }];
    if (id) {
        clauses.push(
            { lead_id: `twilio:${id}` },
            { "twilio.call_sid": id },
            { lead_id: `telnyx:${id}` },
            { "telnyx.call_control_id": id },
            { "telnyx.external_call_id": id },
            { "twilio.external_call_id": id }
        );
    }
    return { $or: clauses };
}
...
async function loadCallLogDoc(callId) {
    ...
    const filter = buildCallLogLookupFilter(callId);
    // Prefer the doc that already has conversation turns (avoids empty race / mis-route siblings).
    const docs = await db.collection(collectionName).find(filter).limit(8).toArray();
...
async function isTestCallForAnalysis(callId, callLogDoc, options = {}) {
    ...
    try {
        ...
        const hit = await db.collection(testColl).findOne(
            {
                $or: [{ call_id: id }, { lead_id: id }, { call_unique_id: id }],
            },
            { projection: { _id: 1 } }
        );
        return !!hit;
...
async function triggerCallAnalysis(callId, options = {}) {
    ...
    // Twilio/Telnyx: webhook owns analysis after conversation store.
    // India/pool: pool conversation endpoint also triggers here as a safety net when the
    // dialer worker misses post-call (e.g. contact left processing with callReceiveStatus=3).
    // Worker still runs India analysis via post-call; Redis lock prevents duplicate API calls.
    const allowedProviders = new Set(["twilio", "telnyx", "india", "pool"]);
