/**
 * POST to ANALYSIS_API_URL for foreign outbound (Twilio + Telnyx) conversation webhooks.
 * India/pool analysis is not triggered here — Calling_system1 owns the India post-call pipeline.
 *
 ...
 *
 * script.ondial.ai / sscript.ondial.ai (v1) — also forced by CAMPAIGN_SERVICE_CATALOG_UI_SCRIPT=true
 * regardless of hostname:
 *   POST /v1/analysis/call — JSON body { payload: ... }, header x-header-key: 1
 *
...
const RETRYABLE_ANALYSIS_HTTP_STATUSES = new Set([500, 502, 503, 504]);
const {
    isUuidCallId,
    isTwilioCallSid,
...
const { sanitizeAnalysisPayloadForV1Api, normalizeConversationTurns } = require("./analysis_services/shared");
const { mapWhatsappTemplateDocForAnalysis } = require("./analysis_services/whatsappSampleValues");
const {
...
function isRetryableAnalysisFailure(status, responseBody = "") {
    if (RETRYABLE_ANALYSIS_HTTP_STATUSES.has(Number(status))) return true;
    if (status === 404 || /not\s*found/i.test(String(responseBody || ""))) return true;
    // Only retry 422 when API complained about empty conversation_text (race), not schema misses.
    if (Number(status) === 422 && /conversation_text/i.test(String(responseBody || ""))) return true;
    return false;
}
...
/** CAMPAIGN_SERVICE_CATALOG_UI_SCRIPT=true forces the v1 script.ondial.ai routing below
 *  (correct path + payload wrapping + x-header-key), regardless of the exact hostname
 *  configured in ANALYSIS_API_URL. Also read as NEXT_PUBLIC_CAMPAIGN_SERVICE_CATALOG_UI_SCRIPT
 *  for parity with the header-only check further down. */
function isScriptCatalogUiScriptEnabled() {
    return (
...
/** True when ANALYSIS_API_URL targets the v1 script service (sscript.ondial.ai / script.ondial.ai). */
function isV1AnalysisApi(raw) {
    const s = String(raw || "");
    return s.includes("/v1") || s.includes("sforeignscript.ondial.ai") || s.includes("foreignscript.ondial.ai") || isScriptCatalogUiScriptEnabled();
}
...
function buildAnalysisUrl(callId, { bodyPayloadMode = false } = {}) {
    const raw = String(process.env.ANALYSIS_API_URL || "https://foreignscript.ondial.ai").replace(/\/$/, "");
    const base = normalizeAnalysisBaseUrl(raw);
...
function detectCallProvider(inputCallId, callLogDoc) {
    // Telnyx first — foreign dialer may return call_control_id as call_sid (v3:...).
    if (
        isTelnyxCallControlId(inputCallId) ||
        pickString(callLogDoc?.telnyx?.call_control_id) ||
        isTelnyxCallControlId(callLogDoc?.call_id) ||
        isTelnyxCallControlId(callLogDoc?.telnyx?.call_control_id)
    ) {
        return "telnyx";
    }
    if (isTwilioCallSid(inputCallId) || isTwilioCallSid(callLogDoc?.twilio?.call_sid)) {
        return "twilio";
    }
    if (pickString(callLogDoc?.twilio?.call_sid) && !isTelnyxCallControlId(callLogDoc.twilio.call_sid)) {
        return "twilio";
    }
    if (isUuidCallId(inputCallId)) {
        // Prefer doc provider when UUID is the dialer id for a foreign call.
        if (pickString(callLogDoc?.telnyx?.call_control_id)) return "telnyx";
        if (pickString(callLogDoc?.twilio?.call_sid)) return "twilio";
        if (String(callLogDoc?.provider || "").toLowerCase() === "pool") return "india";
        return "india";
    }
    if (callLogDoc?.telnyx?.call_control_id) {
        return "telnyx";
    }
...
function countTurns(callLogDoc) {
    ...
    const twilio = Array.isArray(callLogDoc?.twilio?.conversation?.turns)
        ...
        : 0;
    const telnyx = Array.isArray(callLogDoc?.telnyx?.conversation?.turns)
        ? callLogDoc.telnyx.conversation.turns.length
        : 0;
    return Math.max(direct, twilio, telnyx);
}
...
async function loadCallLogDoc(callId) {
    ...
    // Prefer the doc that already has conversation turns (avoids empty race / mis-route siblings).
    // Sequential indexed finds — avoids Atlas COLLSCAN from 3-way $or on call_unique_id.
    const docs = await findCallLogsByIdentity(db.collection(collectionName), callId, {
        limitPerQuery: 8,
        includeCarrier: true,
    });
    if (!docs.length) return null;
    docs.sort((a, b) => countTurns(b) - countTurns(a));
    return docs[0];
}
...
async function resolveAnalysisPayload(callId) {
    ...
    if (campaignId && /^[a-f0-9]{24}$/i.test(campaignId)) {
        ...
        if (campaignDoc && campaignDoc.followup === true && Array.isArray(campaignDoc.followupWhatsappTemplateIds) && campaignDoc.followupWhatsappTemplateIds.length > 0) {
            try {
                const templateIds = campaignDoc.followupWhatsappTemplateIds
                    .map((id) => {
                        try {
                            return typeof id === "string" || id?._id ? new ObjectId(String(id._id ?? id)) : id;
                        } catch {
                            return null;
                        }
                    })
                ...
                const [owned, platform] = await Promise.all([
                    db.collection("whatsapptemplates").find({ _id: { $in: templateIds } }).toArray(),
                    db.collection("platform_whatsapp_templates").find({ _id: { $in: templateIds } }).toArray(),
                ]);
                const byId = new Map([...owned, ...platform].map((t) => [String(t._id), t]));
                campaignDoc.whatsappTemplates = templateIds
                    .map((oid) => byId.get(String(oid)))
                    .filter(Boolean)
                    .map((t) => mapWhatsappTemplateDocForAnalysis(t));
            } catch (wError) {
                logger.warn("[Analysis] Failed to populate whatsappTemplates for analysis payload", { error: wError.message });
            }
        }
        if (campaignDoc) {
            try {
...
async function isTestCallForAnalysis(callId, callLogDoc, options = {}) {
    if (options.isTestCall === true || options.is_test_call === true) return true;
    if (callLogDocIsTestCall(callLogDoc)) return true;
    const id = String(callId || "").trim();
    if (!id) return false;
    try {
        const db = getDb();
        const testColl = process.env.TESTCALL_COLLECTION || "TestCall";
        const hits = await findCallLogsByIdentity(db.collection(testColl), id, {
            limitPerQuery: 1,
...
async function triggerCallAnalysis(callId, options = {}) {
    ...
    if (await isTestCallForAnalysis(id, callLogDocForProvider, options)) {
        logger.info("[Analysis] Skipped — test call (not written to call_analysis)", {
            callId: id,
            callLogIsTest: callLogDocIsTestCall(callLogDocForProvider),
        });
        return { triggered: false, reason: "test_call" };
    }

    const provider = detectCallProvider(id, callLogDocForProvider);
    // Twilio/Telnyx only by default: webhook owns foreign analysis (ANALYSIS_API_URL / foreignscript).
    // India/pool analysis is owned by Calling_system1 post-call.
    // Opt-in safety net: WEBHOOK_TRIGGER_INDIA_ANALYSIS=true (still uses this host's ANALYSIS_API_URL).
    const allowIndiaOnWebhook =
        String(process.env.WEBHOOK_TRIGGER_INDIA_ANALYSIS || "")
            .trim()
            .toLowerCase() === "true";
    const allowedProviders = new Set(["twilio", "telnyx"]);
    if (allowIndiaOnWebhook) {
        allowedProviders.add("india");
        allowedProviders.add("pool");
    }
    if (!allowedProviders.has(provider)) {
        logger.info("[Analysis] Skipped — unsupported call provider", {
            callId: id,
            callProvider: provider,
            reason:
                provider === "india" || provider === "pool"
                    ? "india_owned_by_cs1"
                    : "unsupported_provider",
            webhookIndiaAnalysis: allowIndiaOnWebhook,
        });
        return { triggered: false, reason: "unsupported_provider", callProvider: provider };
    }
    ...
    try {
        ...
        if (isScriptCatalogUiScriptEnabled()) {
        ...
        const applyBodyPayload = (payload, modeLabel) => {
            ...
            if (isV1) {
                ...
                finalPayload = sanitizeAnalysisPayloadForV1Api(finalPayload, campaignDoc);
            }
        ...
        if (ANALYSIS_USE_BODY_PAYLOAD || forceBodyPayload) {
            logger.info("[Analysis] Route: body payload — Twilio + Telnyx", {
                callId: id,
            });
            ...
            analysisCallId = resolveLegacyAnalysisCallId(callLogDoc, id) || id;
            if (!payload) {
            ...
            if (res.ok) {
                ...
                try {
                    ...
                    if (analysisResult) {
                        ...
                        const persistCallId = analysisCallId || id;
                        const updateObj = {
                            call_id: persistCallId,
                            internalCallId: persistCallId,
                            input_call_id: id,
                            call_provider: callProvider,
                            carrier_call_id:
                                callProvider === "telnyx"
                                    ? pickString(
                                          callLogDocForProvider?.telnyx?.call_control_id,
                                          isTelnyxCallControlId(id) ? id : null
                                      )
                                    : callProvider === "twilio"
                                      ? pickString(
                                            callLogDocForProvider?.twilio?.call_sid,
                                            isTwilioCallSid(id) ? id : null
                                        )
                                      : null,
                        ...
                        logger.info("[Analysis] Persisted analysis to call_analysis collection", {
                        ...
                        if (userIdObj) {
                            try {
                                const { notifyTenantCallAnalysis } = require("./notifyTenantCallStatus");
                                void notifyTenantCallAnalysis({
                                    userId: userIdObj,
                                    contactId: contactDoc?._id || null,
                                    campaignId: campaignDoc?._id || null,
                                    callId: analysisInputCallId || persistCallId || id,
                                });
                            } catch (notifyErr) {
                                logger.warn("[Analysis] tenant analysis notify failed", {
