...
async function resolveAnalysisPayload(callId) {
    ...
    const callLogDoc = await db.collection(collectionName).findOne(buildCallLogLookupFilter(callId));
...
async function triggerCallAnalysis(callId, options = {}) {
    ...
    try {
        ...
        } else if (deferIfNoTurns) {
            // Legacy API reads Mongo; optional defer is a no-op (retries handle 404).
            logger.debug("[Analysis] Legacy URL mode — deferIfNoTurns ignored", { callId: id });
        }
