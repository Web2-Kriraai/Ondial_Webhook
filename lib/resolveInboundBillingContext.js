...
async function resolveInboundBillingContext({ anchor, toPhone }) {
    let campaignId = extractConfigIdFromDoc(anchor?.doc) || anchor?.campaignId || null;
    let userId = anchor?.userId || objectIdToString(anchor?.doc?.userId);
    let source = campaignId ? "inbound_doc" : null;
