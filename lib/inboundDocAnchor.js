...
/** UUID and call_<hex> forms so validate stubs and UI rows both match. */
function callSidLookupVariants(callSidOrProviderId) {
    const raw = String(callSidOrProviderId || "").trim();
    if (!raw) return [];
    const out = new Set([raw]);
    const asCall = providerCallIdToCallSid(raw);
    if (asCall) out.add(asCall);
    const asUuid = callSidToProviderCallId(raw);
    if (asUuid) out.add(asUuid);
    if (isProviderUuidCallId(raw)) out.add(raw.toLowerCase());
    return [...out];
}
...
async function findValidateCompanionDoc(db, { callSid, toPhone, fromPhone }) {
    ...
    const sidVariants = callSidLookupVariants(callSid);
    if (sidVariants.length) {
        const bySid = await db
            .collection(INBOUNDCALLLOG_COLLECTION)
            .find({ call_sid: { $in: sidVariants } })
            .toArray();
...
/** InboundConversation row with userId — bill/update on this doc (call_sid link). */
async function findInboundAccountDoc(db, { callSid, toPhone, fromPhone }) {
    const candidates = [];
    ...
    const uiDoc = await findValidateCompanionDoc(db, { callSid, toPhone, fromPhone });
    if (uiDoc) candidates.push(uiDoc);
...
async function ensureInboundDocMetadata(db, doc, { callSid, toPhone, fromPhone, campaignId, userId }) {
    ...
    if (to) set.to_number = to;
    if (from) set.from_number = from;
    // Validate rows keep bot config_id — billing campaign is separate (DID lookup).
    if (campaignId && doc?.source !== "validate") set.config_id = String(campaignId);
    if (userId) set.userId = String(userId);

    if (!Object.keys(set).length) return;
    set.updatedAt = new Date();
    const variants = callSidLookupVariants(sid);
    await db.collection(INBOUNDCALLLOG_COLLECTION).updateMany(
        { call_sid: { $in: variants.length ? variants : [sid] } },
        { $set: set }
    );
}
...
async function resolveInboundConversationAnchor(providerCallId, context = {}) {
    ...
    try {
        ...
        for (const v of callSidLookupVariants(callSid || providerId)) {
            or.push({ call_sid: v });
            or.push({ call_id: v });
        }

        let docs = await db.collection(INBOUNDCALLLOG_COLLECTION).find({ $or: or }).toArray();
        const accountDoc = await findInboundAccountDoc(db, { callSid, toPhone, fromPhone });
        if (accountDoc) {
            docs = mergeInboundDocs(docs, [accountDoc]);
        }

        const doc = accountDoc || pickBestInboundDoc(docs);
        ...
        if (callSid && (context.campaignId || context.userId || toPhone || fromPhone)) {
            await ensureInboundDocMetadata(db, doc, {
                callSid,
                toPhone,
                fromPhone,
                campaignId: context.campaignId,
                userId: context.userId || objectIdToString(doc.userId),
            });
        }
        ...
        return {
            ...
            userId: objectIdToString(doc.userId) || objectIdToString(context.userId),
        };
    } catch (err) {
...
async function syncInboundCompletionFields({
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const validateDoc = await findInboundAccountDoc(db, { callSid, toPhone, fromPhone });
        if (validateDoc) {
    ...
    if (callSid) {
        const variants = callSidLookupVariants(callSid);
        await db.collection(INBOUNDCALLLOG_COLLECTION).updateMany(
            { call_sid: { $in: variants.length ? variants : [String(callSid)] } },
            { $set: set }
        );
    }
