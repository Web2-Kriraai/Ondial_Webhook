...
async function isWizardTestCallLinked({ callSid, dialerCallId }) {
    const sid = String(callSid || "").trim();
    const dialerId = String(dialerCallId || "").trim();
    if (!sid && !dialerId) return false;
    const db = getDb();
    const TESTCALL_COLLECTION = process.env.TESTCALL_COLLECTION || "TestCall";
    const or = [];
    if (sid) {
        or.push({ call_id: sid }, { provider_call_id: sid }, { lead_id: `twilio:${sid}` });
    }
    // Wizard rows are written by the dialer, so they carry the dialer id, not the CallSid.
    if (dialerId) {
        or.push({ call_id: dialerId }, { lead_id: dialerId });
    }
...
async function maybeDeductTwilioCallCredits({ callSid, durationSec, twilioMapping, body, collectionName }) {
    ...
    const { doc, contactId, campaignId, dialerCallId } = await resolveTwilioBillingIds({
        callSid: sid,
        twilioMapping,
        body,
        collectionName,
    ...
    if (creditTestBypassEnabled()) {
        const wizardTest =
            (await isWizardTestCallLinked({ callSid: sid, dialerCallId })) ||
    ...
    logger.info("[Twilio] Credit deduction", {
        CallSid: sid,
        billing_call_id: dialerCallId || sid,
        contact_id: contactId || null,
