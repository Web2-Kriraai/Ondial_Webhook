...
function durationSecFromTelnyxPayload(payload, answeredAtIso) {
  ...
  // Talk time only — never billed_duration_secs / billable_duration_secs.
  // Telnyx call.cost rounds those to whole minutes (often 60) and must not overwrite hangup.
  const explicit = Number(
    payload?.duration_secs ?? payload?.CallDuration ?? payload?.duration
  );
