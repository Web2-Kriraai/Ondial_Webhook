...
/** Known telephony providers for optional per-country resource cost overrides. */
...
function normalizeProviderResourceCosts(raw) {
...
function normalizeProviders(raw) {
    ...
    for (const key of Object.keys(raw)) {
        ...
        if (!id) continue;
        const normalized = normalizeProviderResourceCosts(raw[key]);
...
function mergeProviders(prevProviders, incomingProviders) {
    ...
    if (incomingProviders && typeof incomingProviders === "object") {
        for (const [pid, costs] of Object.entries(incomingProviders)) {
            merged[pid] = { ...(merged[pid] || {}), ...costs };
        }
    }
