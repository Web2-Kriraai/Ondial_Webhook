...
const LOOKBACK_MS = Number(process.env.TEST_CALL_PHONE_LINK_LOOKBACK_MS || 2 * 60 * 1000);
        ...
        const testDoc = await db.collection(TESTCALL_COLLECTION).findOne(
            {
                ...
                createdAt: { $gte: since },
                isTestCall: true,
                $or: [{ provider_call_id: { $exists: false } }, { provider_call_id: "" }, { provider_call_id: null }],
            },
            { sort: { createdAt: -1 } }
        );
