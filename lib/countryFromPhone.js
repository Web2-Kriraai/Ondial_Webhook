...
/**
 * Phone used for country-based rate lookup.
 * Outbound: callee (to). Inbound: caller (from).
 */
function resolveCountryBillingPhone(callLogDoc, { inbound = false } = {}) {
    const doc = callLogDoc || {};
    if (inbound) {
        return extractCallerPhone(doc) || phoneFromInboundEvents(doc) || null;
    }
    return extractDestinationPhone(doc);
}
