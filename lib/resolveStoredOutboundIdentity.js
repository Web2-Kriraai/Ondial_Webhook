...
async function resolveStoredOutboundIdentity(identity, options = {}) {
    ...
    try {
        ...
        const [callLogDoc, testCallDoc] = await Promise.all([
            findOneCallLogByIdentity(db.collection(CALLLOGS_COLLECTION), key, {
                includeCarrier: false,
                limitPerQuery: 1,
            }),
            findOneCallLogByIdentity(db.collection(TESTCALL_COLLECTION), key, {
                includeCarrier: false,
                limitPerQuery: 1,
            }),
        ]);
        ...
        if (options.backfillCallLog !== false && merged.campaign_id) {
            ...
            // Prefer the CallLogs doc we already loaded; otherwise match by dialer identity.
            const filter = callLogDoc?._id
                ? { _id: callLogDoc._id }
                : {
                      $or: [
                          { lead_id: key },
